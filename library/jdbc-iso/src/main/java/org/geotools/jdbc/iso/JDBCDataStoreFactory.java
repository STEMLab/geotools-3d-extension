...
public abstract class JDBCDataStoreFactory implements DataStoreFactorySpi {
	...
	/**
	 * If different from one, the JDBCInsertFeatureWriter will buffer the
	 * features and insert them in batches
	 */
	public static final Param BATCH_INSERT_SIZE = new Param("Batch insert size", Integer.class,
			"Number of records inserted in the same batch (default, 1). For optimal performance, set to 100.", false,
			1);

	/**
	 * Maximum amount of time the pool will wait when trying to grab a new
	 * connection
	 **/
	...
	public static final Param EVICTOR_TESTS_PER_RUN = new Param("Evictor tests per run", Integer.class,
			"number of connections checked by the idle connection evictor for each of its runs (defaults to 3)", false,
			3);
	...
	/**
	 * Number of prepared statements cached per connection (this param is
	 * exposed only by factories supporting prepared statements
	 **/
	...
	public static final Param EXPOSE_PK = new Param("Expose primary keys", Boolean.class,
			"Expose primary key columns as " + "attributes of the feature type", false, false);
	 ...
	 * <li>Everything is of the correct type (or upcovertable to the correct
	 * type without Error)
	 ...
	 * 
	 * @param params
	 * @return true if params is in agreement with getParametersInfo and
	 *         checkDBType
	 */
	...
	/**
	 * Subclass hook to do additional initialization of a newly created
	 * datastore.
	 * <p>
	 ...
	 * subclasses may wish to create a new instance of the datastore, for
	 * instance in order to wrap the original instance. This is supported but
	 * the new datastore must be returned from this method. If not is such the
	 * case this method should still return the original passed in.
	 *
	 * </p>
	 * 
	 * @param dataStore
	 ...
	 * <p>
	 * Subclasses may with to override or extend this method. This
	 * implementation checks whether the jdbc driver class (provided by
	 * {@link #getDriverClassName()} can be loaded.
	 * </p>
	 */
	 ...
	 * <li>url -> 'jdbc:&lt;{@link #getDatabaseID()}>://&lt;{@link #HOST}>/&lt;
	 * {@link #DATABASE}>'
	...
	/**
	 * DataSource access allowing SQL use: intended to allow client code to
	 * query available schemas.
	 * <p>
	 * This DataSource is the clients responsibility to close() when they are
	 * finished using it.
	 * </p>
	 * 
	 * @param params
	...
	/**
	 * Override this to return a good validation query (a very quick one, such
	 * as one that asks the database what time is it) or return null if the
	 * factory does not support validation.
	 * 
	 * @return
	 */
	...
	/**
	 * Builds up the JDBC url in a jdbc:<database>://<host>:<port>/
	 * <dbname> Override if you need a different setup
	 * 
	 * @param params
	 * @return
	 * @throws IOException
