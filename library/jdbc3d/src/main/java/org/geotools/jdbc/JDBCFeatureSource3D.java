 ...
 * @source $URL$
 */
public class JDBCFeatureSource3D extends ContentFeatureSource {
    
    private static final Logger LOGGER = Logging.getLogger(JDBCFeatureSource3D.class);
     ...
     */
    public JDBCFeatureSource3D(ContentEntry entry,Query query) throws IOException {
        super(entry,query);
        
        //TODO: cache this
        primaryKey = ((JDBCDataStore3D) entry.getDataStore()).getPrimaryKey(entry);
    }
     ...
     * @throws IOException
     */
    protected JDBCFeatureSource3D(JDBCFeatureSource3D featureSource) throws IOException{
    ...
    /**
     * Type narrow to {@link JDBCDataStore3D}.
     */
    public JDBCDataStore3D getDataStore() {
        return (JDBCDataStore3D) super.getDataStore();
    }
    ...
    protected SimpleFeatureType buildFeatureType() throws IOException {
        ...
        try {
            ...
            for (ColumnMetadata column : columns) {
                ...
                ab.addUserData(JDBCDataStore3D.JDBC_NATIVE_TYPENAME, column.typeName);
                ...
                if (Geometry.class.isAssignableFrom(binding)) {
                    ...
                    ab.setCRS(crs);
                    if(srid != null) {
                        ab.addUserData(JDBCDataStore3D.JDBC_NATIVE_SRID, srid);
                    }
                ...
                if (pkey.getColumn(att.getLocalName()) != null) {
                    att.getUserData().put(JDBCDataStore3D.JDBC_PRIMARY_KEY_COLUMN, true);
                }
            ...
            if(readOnly) {
                ft.getUserData().put(JDBCDataStore3D.JDBC_READ_ONLY, Boolean.TRUE);
            }
    ...
    Filter[] splitFilter(Filter original, FeatureSource source) {
        JDBCFeatureSource3D featureSource = null;
        if (source instanceof JDBCFeatureSource3D) {
            featureSource = (JDBCFeatureSource3D) source;
        }
        else {
            featureSource = ((JDBCFeatureStore3D)source).getFeatureSource();
        }
    ...
    protected int getCountInternal(Query query) throws IOException {
        JDBCDataStore3D dataStore = getDataStore();
