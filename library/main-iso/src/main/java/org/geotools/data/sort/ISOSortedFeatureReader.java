 ...
 * @source $URL$
 */
public class ISOSortedFeatureReader implements SimpleFeatureReader {
    ...
    public static final boolean canSort(SimpleFeatureType schema, SortBy[] sortBy) {
        return ISOMergeSortDumper.canSort(schema, sortBy);
    }
    ...
    public static int getMaxFeaturesInMemory(Query q) {
        return ISOMergeSortDumper.getMaxFeatures(q);
    }
     ...
     */
    public ISOSortedFeatureReader(SimpleFeatureReader reader, Query query) throws IOException {
        this.delegate = ISOMergeSortDumper.getDelegateReader(reader, query);
    }
     ...
     * @throws IOException
     */
    public ISOSortedFeatureReader(SimpleFeatureReader reader, SortBy[] sortBy, int maxFeatures)
            throws IOException {
        this.delegate = ISOMergeSortDumper.getDelegateReader(reader, sortBy, maxFeatures);
    }
