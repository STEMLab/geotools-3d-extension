...
public class CoordinateWriter {
     ...
     * @throws SAXException
     */
    public void writeCoordinates(PointArray c, ContentHandler output)
    	throws SAXException {
        ...
        final int coordSeqDimension = c.getCoordinateReferenceSystem().getCoordinateSystem().getDimension();
        double x, y, z;
        //write down a coordinate at a time
        for (int i = 0, n = coordCount; i < n; i++) {
        	DirectPosition dp = c.get(i).getDirectPosition();
            x = dp.getOrdinate(0);
            y = dp.getOrdinate(1);
