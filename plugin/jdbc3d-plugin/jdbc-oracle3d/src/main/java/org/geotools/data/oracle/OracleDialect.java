...
public class OracleDialect extends PreparedStatementSQLDialect {
    ...
    public Envelope decodeGeometryEnvelope(ResultSet rs, int column, Connection cx )
    	...
    	Object geom = readGeometry(rs, column, new ISOGeometryBuilder(DefaultGeographicCRS.WGS84), cx);

    	if(geom instanceof Geometry)
    		return geom != null ? ((Geometry)geom).getEnvelope() : null;
		else 
			return null;	
    }
	...
	public Geometry decodeGeometryValue(GeometryDescriptor descriptor, ResultSet rs, String column,
        ...
        return (Geometry) convertGeometry(geom, descriptor, factory);
	}
	...
	private Object convertGeometry(Object geom, GeometryDescriptor descriptor, ISOGeometryBuilder factory) {
        ...
        if(targetClazz.equals(MultiSurface.class) && (geom instanceof Surface || geom instanceof Polygon)){
        	MultiPrimitive result = factory.createMultiPrimitive();
            Set elements = result.getElements();
            elements.add(geom);
            return factory.createMultiSurface(elements);
        }
        else if(targetClazz.equals(MultiPoint.class) && geom instanceof Point) {
        	MultiPrimitive result = factory.createMultiPrimitive();
            Set elements = result.getElements();
            elements.add(geom);
            return factory.createMultiPoint(elements);
        }
        else if(targetClazz.equals(MultiCurve.class) && (geom instanceof Curve || geom instanceof LineString)) {
        	MultiPrimitive result = factory.createMultiPrimitive();
            Set elements = result.getElements();
            elements.add(geom);
            return factory.createMultiCurve(elements);
        }
        else if(targetClazz.equals(MultiPrimitive.class)) {
        	MultiPrimitive result = factory.createMultiPrimitive();
            Set elements = result.getElements();
            elements.add(geom);
            return factory.createMultiPrimitive(elements);
        }
        
        return geom;
    }
    
    Object readGeometry(ResultSet rs, String column, ISOGeometryBuilder factory, Connection cx)
            throws IOException, SQLException {
        return readGeometry(rs.getObject(column), factory, cx);
    }

    Object readGeometry(ResultSet rs, int column, ISOGeometryBuilder factory, Connection cx)
            throws IOException, SQLException {
        return readGeometry(rs.getObject(column), factory, cx);
    }

    Object readGeometry(Object struct, ISOGeometryBuilder factory, Connection cx)
            throws IOException, SQLException {
        if (struct == null) {
            return null;
        }
...
*/
                    ...
                    if (att instanceof GeometryDescriptor) {
                        ...
                        if (canAccessUserViews(cx)) {
                            ...
                            if (rs.next()) {
                                ...
                                // reproject and merge
                                if (env != null) {
                                    result.add((ReferencedEnvelope) env);
                                    rs.close();
                                    continue;
                                }
                            }
            ...
            for (AttributeDescriptor att : featureType.getAttributeDescriptors()) {
                if (att instanceof GeometryDescriptor) {
                    ...
                    if (rs.next()) {
                        ...
                        Geometry geometry = (Geometry) readGeometry(rs, 1, new ISOGeometryBuilder(descriptor.getCoordinateReferenceSystem()), cx);
                        
                        if(geometry instanceof Envelope){
                        	result.add((ReferencedEnvelope)geometry);
                        }
                    }
                    rs.close();
                }
    ...
    private Envelope decodeDiminfoEnvelope(ResultSet rs, int column) throws SQLException
    {
        ...
        return builder.createEnvelope(builder.createDirectPosition(new double[]{minx, miny}),
        		builder.createDirectPosition(new double[]{maxx, maxy}));
    }
}
