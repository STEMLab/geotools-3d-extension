...
class SDO11OracleDialect extends OracleDialect {
    ...
    @Override
    Geometry readGeometry(ResultSet rs, String column, ISOGeometryBuilder factory, Connection cx)
            throws IOException, SQLException {
        return readGeometry(rs.getBytes(column), factory, cx);
    }

    @Override
    Geometry readGeometry(ResultSet rs, int column, ISOGeometryBuilder factory, Connection cx)
            throws IOException, SQLException {
        return readGeometry(rs.getBytes(column), factory, cx);
    }

    private Geometry readGeometry(byte[] rawGeometry, ISOGeometryBuilder factory, Connection cx) throws SQLException {
        if(rawGeometry == null)
            return null;
