...
import org.geotools.jdbc.iso.JDBCTestSupport;
...
public class SDOOnlineTest extends JDBCTestSupport {
    ...
    final public void testDecodeSolid() throws SQLException {
        if (this.connection == null)
            return;
        
        ISOGeometryBuilder prevbuilder = converter.geometryFactory;
        // 3-Dimension case
        Hints hints = GeoTools.getDefaultHints();
        hints.put(Hints.CRS, DefaultGeographicCRS.WGS84_3D);
        hints.put(Hints.GEOMETRY_VALIDATE, false);
        ISOGeometryBuilder builder = new ISOGeometryBuilder(hints);
        converter.geometryFactory = builder;
        
        STRUCT datum = converter.toSDO(fixture.solid);
        Geometry geom = (Geometry) converter.asGeometry(datum);

        assertEquals(fixture.solid, geom);
        
        converter.geometryFactory = prevbuilder;
    }
    ...
    final public void testGeometryCollection2() throws Exception {
        ...
        String wkt = "MULTIPRIMITIVE (LINESTRING (679572.8376 5151850.0275, 679583.1288 5151850.8366, "
        ...
        Geometry original = new WKTReader(DefaultGeographicCRS.WGS84).read(wkt);
        ((GeometryImpl)original).setUserData(25832); // Set SRID
        STRUCT datum = converter.toSDO(original);
        Geometry geom = (Geometry) converter.asGeometry(datum);
        assertEquals(25832, ((GeometryImpl)geom).getUserData());
    ...
    final public void testGeometryCollection3() throws Exception {
        ...
        String wkt = "MULTIPRIMITIVE (POLYGON ((0 0, 10 0, 10 10, 0 10, 0 0)), POINT (5 5))"; 
        Geometry original = new WKTReader(DefaultGeographicCRS.WGS84).read(wkt);
        ((GeometryImpl)original).setUserData(4326);
        STRUCT datum = converter.toSDO(original);
        Geometry geom = (Geometry) converter.asGeometry(datum);
        assertEquals(4326, ((GeometryImpl)original).getUserData());
        assertEquals(original, geom);
    }
    ...
    final public void testGeometryCollectionMultipoint() throws Exception {
        ...
        String wkt = "MULTIPRIMITIVE (POLYGON ((0 0, 10 0, 10 10, 0 10, 0 0)), MULTIPOINT ((5 5), (10 10)))"; 
        Geometry original = new WKTReader(DefaultGeographicCRS.WGS84).read(wkt);
        ((GeometryImpl)original).setUserData(4326);
        STRUCT datum = converter.toSDO(original);
        Geometry geom = (Geometry) converter.asGeometry(datum);
