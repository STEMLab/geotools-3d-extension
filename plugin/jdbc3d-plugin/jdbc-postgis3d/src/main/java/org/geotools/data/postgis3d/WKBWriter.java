 ...
 * {@link OutStream}, which allows easy use with arbitary byte stream sinks.
 * <p>
 * The WKB format is specified in the OGC
 * <A HREF="http://www.opengis.org/techno/specs.htm"><i>Simple Features for
 * SQL</i></a> specification. This implementation also supports the <b>Extended
 * WKB</b> standard. Extended WKB allows writing 3-dimensional coordinates and
 * including the geometry SRID value. The presence of 3D coordinates is
 * signified by setting the high bit of the <tt>wkbType</tt> word. The presence
 * of an SRID is signified by setting the third bit of the <tt>wkbType</tt>
 * word. EWKB format is upward compatible with the original SFS WKB format.
 * <p>
 ...
 * will be written as {@link LineString}s.
 * <p>
 ...
 * Well-Known Binary supported by JTS.
 * <p>
 * <i>The specification uses a syntax language similar to that used in the C
 * language. Bitfields are specified from hi-order to lo-order bits.</i>
 * <p>
 * <blockquote>
 * 
 * <pre>
...
public class WKBWriter {
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with output dimension = 2
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension
	 * (2 or 3) for output coordinates and {@link ByteOrderValues#BIG_ENDIAN}
	 * byte order. If the input geometry has a small coordinate dimension,
	 ...
	 * Creates a writer that writes {@link Geometry}s with the given dimension
	 * (2 or 3) for output coordinates and {@link ByteOrderValues#BIG_ENDIAN}
	 * byte order. This constructor also takes a flag to control whether srid
	 * information will be written. If the input geometry has a smaller
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension
	 * (2 or 3) for output coordinates and byte order If the input geometry has
	 * a small coordinate dimension, coordinates will be padded with
	 * {@link Coordinate#NULL_ORDINATE}.
	 *
	 * @param outputDimension
	 *            the coordinate dimension to output (2 or 3)
	...
	/**
	 * Creates a writer that writes {@link Geometry}s with the given dimension
	 * (2 or 3) for output coordinates and byte order. This constructor also
	 * takes a flag to control whether srid information will be written. If the
	 * input geometry has a small coordinate dimension, coordinates will be
	...
	public void write(Geometry geom, OutStream os) throws IOException {
		...
		/*
		 * else if (geom instanceof LineString) writeLineString((LineString)
		 * geom, os); else if (geom instanceof Polygon) writePolygon((Polygon)
		 * geom, os); else if (geom instanceof MultiPoint)
		 * writeGeometryCollection(WKBConstants.wkbMultiPoint, (MultiPoint)
		 * geom, os); else if (geom instanceof MultiLineString)
		 * writeGeometryCollection(WKBConstants.wkbMultiLineString,
		 * (MultiLineString) geom, os); else if (geom instanceof MultiPolygon)
		 * writeGeometryCollection(WKBConstants.wkbMultiPolygon, (MultiPolygon)
		 * geom, os); else if (geom instanceof GeometryCollection)
	...
	private void writePoint(Point pt, OutStream os) throws IOException {
		...
		// throw new IllegalArgumentException("Empty Points cannot be
		// represented in WKB");
		writeByteOrder(os);
		writeGeometryType(WKBConstants.wkbPoint, pt, os);
		writeCoordinate(pt.getDirectPosition().getCoordinate(), os);
	}

	/*
	 * private void writeLineString(LineString line, OutStream os) throws
	 * IOException { writeByteOrder(os);
	 ...
	 * { writeByteOrder(os); writeGeometryType(WKBConstants.wkbPolygon, poly,
	 * os); writeInt(poly.getNumInteriorRing() + 1, os);
	 * writeCoordinateSequence(poly.getExteriorRing().getCoordinateSequence(),
	 * true, os); for (int i = 0; i < poly.getNumInteriorRing(); i++) {
	 * writeCoordinateSequence(poly.getInteriorRingN(i).getCoordinateSequence(),
	 * true, os); } }
	 * 
	 * private void writeGeometryCollection(int geometryType, GeometryCollection
	 * gc, OutStream os) throws IOException { writeByteOrder(os);
	 * writeGeometryType(geometryType, gc, os); writeInt(gc.getNumGeometries(),
	 * os); for (int i = 0; i < gc.getNumGeometries(); i++) {
	...
	private void writeCoordinateSequence(DirectPosition[] seq, boolean writeSize, OutStream os) throws IOException {
		if (writeSize)
			writeInt(seq.length, os);

		for (int i = 0; i < seq.length; i++) {
			writeCoordinate(seq[i].getCoordinate(), os);
		}
	}

	private void writeCoordinate(double[] seq, OutStream os) throws IOException {
		ByteOrderValues.putDouble(seq[0], buf, byteOrder);
		os.write(buf, 8);
		ByteOrderValues.putDouble(seq[1], buf, byteOrder);
		os.write(buf, 8);
