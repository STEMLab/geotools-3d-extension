 ...
 */
package org.geotools.iso.geojson.feature;
...
import org.geotools.iso.geojson.geom.ISOGeometryHandler;
...
public class FeatureHandler extends DelegatingHandler<SimpleFeature> {
    ...
    public FeatureHandler(ISOSimpleFeatureBuilder builder, AttributeIO attio) {
    ...
    public boolean endObject() throws ParseException, IOException {
        if (delegate instanceof IContentHandler) {
            ...
            if (delegate instanceof ISOGeometryHandler) {
                Geometry g = ((IContentHandler<Geometry>) delegate).getValue();
                if (g == null && ((ISOGeometryHandler) delegate)
                        .getDelegate() instanceof MultiPrimitiveHandler) {
    ...
    public boolean primitive(Object value) throws ParseException, IOException {
        if (delegate instanceof ISOGeometryHandler && value == null) {
            delegate = NULL;
            return true;
        } else if ("".equals(id)) {
            id = value.toString();
    ...
    ISOSimpleFeatureBuilder createBuilder() {
        ...
        return new ISOSimpleFeatureBuilder(typeBuilder.buildFeatureType());
    }

    void addGeometryType(ISOSimpleFeatureTypeBuilder typeBuilder, Geometry geometry) {
    ...
    SimpleFeature buildFeature() {
        ...
        ISOSimpleFeatureBuilder builder = this.builder != null ? this.builder : createBuilder();
        ...
        if (geometry != null) {
            if (featureType.getGeometryDescriptor() == null) {
                ...
                ISOSimpleFeatureBuilder newBuilder = new ISOSimpleFeatureBuilder(featureType);
                newBuilder.init(f);
