 ...
 */
package org.geotools.geometry.iso.topograph;
...
public class Edge extends GraphComponent {
	...
	public static void updateIM(Label label, IntersectionMatrix im) {
		im.setAtLeastIfValid(label.getLocation(0, Position.ON), label
				.getLocation(1, Position.ON), 1);
		if (label.isArea()) {
			im.setAtLeastIfValid(label.getLocation(0, Position.LEFT), label
					.getLocation(1, Position.LEFT), 2);
			im.setAtLeastIfValid(label.getLocation(0, Position.RIGHT), label
					.getLocation(1, Position.RIGHT), 2);
		}
	}
	...
	/**
	 * An Edge is collapsed if it is an Area edge and it consists of two
	...
	/**
	 * Adds EdgeIntersections for one or both intersections found for a segment
	 * of an edge to the edge intersection list.
	 */
	 ...
	 * <p>
	 * e1 equals e2 <b>iff</b> the coordinates of e1 are the same or the
