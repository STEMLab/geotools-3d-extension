...
public class IntersectionMatrix implements Cloneable {
	...
	public void set(String dimensionSymbols) {
		for (int i = 0; i < dimensionSymbols.length(); i++) {
			int row = i / 3;
			int col = i % 3;
			matrix[row][col] = Dimension.toDimensionValue(dimensionSymbols
					.charAt(i));
		}
	}
	 ...
	 *            this <code>IntersectionMatrix</code>. The order of
	...
	public boolean matches(String requiredDimensionSymbols) {
		...
		for (int ai = 0; ai < 3; ai++) {
			for (int bi = 0; bi < 3; bi++) {
				if (!matches(matrix[ai][bi], requiredDimensionSymbols.charAt(3
						* ai + bi))) {
					return false;
				}
			}
...
//	}
	 ...
	 * 
	 * @return the nine dimension symbols of this
	...
	public String toString() {
		...
		for (int ai = 0; ai < 3; ai++) {
			for (int bi = 0; bi < 3; bi++) {
				buf.setCharAt(3 * ai + bi, Dimension
