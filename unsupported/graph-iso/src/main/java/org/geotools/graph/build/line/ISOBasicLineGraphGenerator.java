 ...
 * @source $URL$
 */
public class ISOBasicLineGraphGenerator implements ISOLineGraphGenerator {

  /** coordinate to node map **/
  private HashMap<DirectPosition,Node> m_coord2node;
  ...
  private ISOGeometryBuilder spatialBuilder;
   ...
   * </p>
   */
  public ISOBasicLineGraphGenerator (ISOGeometryBuilder builder) {
    m_coord2node = new HashMap<DirectPosition, Node>();
    setGraphBuilder(new BasicGraphBuilder());
    spatialBuilder = builder;
  }
  ...
  /**
    ...
    */
   public ISOBasicLineGraphGenerator (ISOGeometryBuilder builder, double tolerance) {
  ...
  public Graphable add(Object obj) {
    ...
    DirectPosition first,last;
    Node n1, n2;
    
    //check first coordinate
    first = line.getStartPoint();
    ...
    protected LineSegment alterLine(LineSegment line, Node n1, Node n2) {
        DirectPosition c1added = ((DirectPosition) n1.getObject());
        DirectPosition c2added = ((DirectPosition) n2.getObject());
        if (!c1added.equals(line.getStartPoint()) || c2added.equals(line.getEndPoint())) {
            return spatialBuilder.createLineSegment(c1added, c2added);
        }
        return line;
    }
  ...
  public Graphable get(Object obj) {
    ...
    Node n1 = retrieveNode(line.getStartPoint());
    Node n2 = retrieveNode(line.getEndPoint());
  ...
  //TODO COMMENT ME!
  public Node getNode(DirectPosition c) {
    return retrieveNode(c);
  }

  public Edge getEdge(DirectPosition c1, DirectPosition c2) {
  ...
  private Node createNode(DirectPosition c) {
    Node node;
    ...
    if (useTolerance()) {
      spatialIndex.insert(new Envelope(c.getOrdinate(0), c.getOrdinate(1), c.getOrdinate(0), c.getOrdinate(1)), c);
    }
    return node;
  }

  private Node retrieveNode(DirectPosition c) {
  ...
  private Node findClosestNodeWithinTolerance(DirectPosition inCoord) {
    double closestDistance = Double.MAX_VALUE;
    DirectPosition closestCoordinate = null;
    List<DirectPosition> list = spatialIndex.query(
            new Envelope(c.getOrdinate(0), c.getOrdinate(1) - tolerance, c.getOrdinate(0), c.getOrdinate(1) + tolerance);
    for (DirectPosition c : list) {
