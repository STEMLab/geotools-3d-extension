...
@DescribeProcess(title = "BarnesSurface3D", description = "Uses Barnes Analysis to compute an interpolated surface over a set of irregular data points.")
            ...
            while (obsIt.hasNext()) {
                ...
                try {
                    ...
                    if (valObj != null) {
                        ...
                        double[] p = geom.getRepresentativePoint().getCoordinate();
                        //srcPt[0] = p.x;
                        //srcPt[1] = p.y;
                        srcPt[0] = p[0];
                        srcPt[1] = p[1];
