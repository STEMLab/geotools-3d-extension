 ...
 * @source $URL$
 */
@DescribeProcess(title = "Buffer3D", description = "Buffers features by a distance value supplied either as a parameter or by a feature attribute. Calculates buffers based on Cartesian distances.")
        ...
        protected SimpleFeatureType buildTargetFeatureType() {
            ...
            for (AttributeDescriptor descriptor : delegate.getSchema().getAttributeDescriptors()) {
                if (!(descriptor.getType() instanceof GeometryTypeImpl)
                    ...
                    builder.setBinding(MultiCurve.class);
                ...
                for (Object value : f.getAttributes()) {
                    if (value instanceof Geometry) {
                        ...
                        if(fDistance != null && fDistance != 0.0) {
                            value = ((Geometry) value).getBuffer(fDistance);
                        } 
                    }
                    fb.add(value);
                }
