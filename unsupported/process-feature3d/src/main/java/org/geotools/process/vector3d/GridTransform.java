...
class GridTransform {
    ...
    public GridTransform(Envelope env, int xSize, int ySize) {
        ...
        this.ySize = ySize;
        //
        double low[] = env.getLowerCorner().getCoordinate();
        double high[] = env.getUpperCorner().getCoordinate();
        double width = high[0] - low[0];
    ...
    public int i(double x) {
        if (isClamped && x > env.getMaximum(0))//env.getMaxX())
            return xSize;
        if (isClamped && x < env.getMinimum(0))//env.getMinX())
            return -1;
        int i = (int) ((x - env.getMinimum(0)) / dx);//env.getMinX()) / dx);
    ...
    public int j(double y) {
        if (isClamped && y > env.getMaximum(1))//env.getMaxY())
            return ySize;
        if (isClamped && y < env.getMinimum(1))//env.getMinY())
            return -1;
        int j = (int) ((y - env.getMinimum(1)) / dy);//env.getMinY()) / dy);
        // have already check x is in bounds, so ensure returning a valid value
